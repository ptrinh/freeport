...
| Notification server + MCP | `packages/freeport-self-hosted/` | `docker compose up -d` — content-blind Web Push/Expo push watcher (`ENABLE_NOTIFY=1`) and a read-only MCP endpoint for agents; point the app's "Notification service URL" at it |
| Telegram bridge | `packages/freeport-self-hosted/` | Same service — set `TELEGRAM_BOT_TOKEN` to relay a market feed into groups + send content-blind pings; add `TELEGRAM_GUEST_KEY_PASSPHRASE` for custodial guest-agent mode. Secrets go in a gitignored `.env`, never the committed compose. See the freeport-self-hosted README. |
