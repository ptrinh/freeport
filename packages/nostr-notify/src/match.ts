...
/**
 * Best geohash for an event. Exported so the caller can compute it ONCE per
 * event and pass it into `matches` — the payload JSON.parse fallback is the
 * expensive part, and re-running it per subscriber made one event cost S
 * parses for S subscribers.
 */
