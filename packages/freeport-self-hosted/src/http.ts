...
/**
 * Build the HTTP app + wire the routes, WITHOUT listening or starting the relay
 * — so the e2e suite can boot the real route stack on an ephemeral port and the
 * process entrypoint (main, below) can add the network bits. Env is read here
 * (not at module load) so tests can set ENABLE_WEB/ENABLE_NOTIFY/… per case.
 */
export function buildApp(): { app: express.Express; notifier: Notifier | null } {
...
/** Process entrypoint: build the app, listen, and start the embedded relay. */
function main(): void {
...
// Run only as the process entrypoint (`node dist/http.js`) — importing this
// module (the e2e suite) just gets buildApp with no listener or relay.
